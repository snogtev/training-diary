...
class CTkDatePicker(ctk.CTkFrame):
    def __init__(self, master=None, **kwargs):
        ...
        self.date_entry = ctk.CTkEntry(self, font=('Russo One', 20))
        today = date.today()
    ...
    "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь"
        ...
        days = [calendar.day_abbr[i].capitalize() for i in range(7)]
        for i, day in enumerate(days):
            lbl = ctk.CTkLabel(self.calendar_frame, font=('Russo One', 20), text=day)
        ...
        for week in range(2, 8):
            for day_col in range(7):
                if week == 2 and day_col < start_day:
                    lbl = ctk.CTkLabel(self.calendar_frame, font=('Russo One', 25), text="")
                    ...
                    if ctk.get_appearance_mode() == "Light":
                        btn = ctk.CTkButton(self.calendar_frame, font=('Russo One', 20), text=str(day), width=3, command=lambda day=day: self.select_date(day), fg_color="transparent", text_color="black", hover_color="#3b8ed0")
                    else:
                        btn = ctk.CTkButton(self.calendar_frame, font=('Russo One', 20), text=str(day), width=3, command=lambda day=day: self.select_date(day), fg_color="transparent")
